...
def set_test(test_iter, model_file):
    ...
    for i in pred_tags:
        ...
        # print(type(i))
        if i not in  ["[CLS]"]:
            new_pre_tag.append(i)
    for i in new_pre_tag:
        if i=="":
            id_list.append("")
        else:
            id_list.append(num)
    ...
    print("预测结果长度为：",new_pre_tag)
    data=pandas.DataFrame(columns=["id","tag"])
    data["tag"]=new_pre_tag
    data["id"]=id_list

    data.to_csv(result_data_dir+"result.csv",index=False,encoding="utf8",sep=",")
...
if __name__ == '__main__':
    ...
    dev_iter = DataIterator(config.batch_size,
                            config.processed_data + 'test.txt',
    ...
    for input_ids_list, input_mask_list, segment_ids_list, label_ids_list, tokens_list in tqdm(dev_iter):
        print("输入当前batch第一个句子的id列表：",input_ids_list[0])
        # print("mask编码：",input_mask_list[0])
        # print("token列表：", tokens_list[0])
        print("对应的标签列表：",label_ids_list[0])   #  输出验证集第一个batch 的第一句话
